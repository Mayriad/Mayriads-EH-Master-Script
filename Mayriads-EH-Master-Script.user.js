...
;(function () {
    ...
    useAutomatedDownloads: {
      ...
      runtime: {
        ...
        unavailableTorrentError: 'A torrent download failed, because the torrent is not available at the moment. ' +
          'Please try again after a few minutes or manually download the personalized torrent instead.',
  ...
  const applyAdditionalFilters = function () {
     ...
     *
     * Note that the displayed titles in gallery lists can be the english/romanized titles, or the original Japanese
  ...
  const useAutomatedDownloads = function () {
    ...
    const addDownloadButtons = function () {
      ...
      if (shortcuts.pageDownloadEnabled) {
        ...
        changePageDownloadState(pageDownloadButton, 'idle')
     ...
     *
     * @param {HTMLDivElement} pageDownloadButton - The page download button to be changed.
     ...
     */
    const changePageDownloadState = function (pageDownloadButton, targetState, errorCount) {
      switch (targetState) {
        case 'idle':
    ...
    const attemptPageDownload = function (clickEvent) {
      ...
      if (typeof clickEvent === 'undefined') {
        ...
        const pageDownloadButton = clickEvent.target
        if (!inPageDownloadMode) {
          ...
          changePageDownloadState(pageDownloadButton, 'downloading')
        } else {
    ...
    const checkPageCompletion = function () {
      ...
      if (errorCount === 0) {
        changePageDownloadState(pageDownloadButton, 'done')
      } else {
        changePageDownloadState(pageDownloadButton, 'failed', errorCount)
      }
      ...
      if (inPageDownloadMode) {
        ...
        } else if (shortcuts.pageRangeDownloadEnabled) {
  ...
  const parseExternalLinks = function () {
    ...
    for (const comment of comments) {
      ...
      // would be treated as part of the URL by the site.
      ...
      for (const url of orderedUrls) {
        ...
        if (/(?:repo|upload)\.e-hentai\.org/.test(url) || !/(?:e-hentai|exhentai)\.org/.test(url)) {
   ...
   * EH gallery view and on the news page. The EH wiki is still not covered at the moment.
   */
